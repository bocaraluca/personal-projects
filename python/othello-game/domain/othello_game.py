from domain.othello_board import OthelloBoard, OthelloSymbol
...
class OthelloGame:
    def __init__(self, human_player):
        ...
        self._computer_player = OthelloSymbol.BLACK.value if human_player == OthelloSymbol.WHITE.value else OthelloSymbol.WHITE.value
    ...
    def get_score(self) -> dict:
        ...
        """
        score = {OthelloSymbol.BLACK.value: 0, OthelloSymbol.WHITE.value: 0}
        for row in self._board.data:
            for cell in row:
                if cell == OthelloSymbol.BLACK.value:
                    score[OthelloSymbol.BLACK.value] += 1
                elif cell == OthelloSymbol.WHITE.value:
                    score[OthelloSymbol.WHITE.value] += 1
        return score
